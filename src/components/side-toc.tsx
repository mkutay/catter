...
export function SideTOC({ toc }: { toc: TocItem[] }) {
  ...
  const renderTocSection = (section: TocSection, level: number = 0): React.ReactElement => {
    ...
    const isChildActive = children.some(child => {
      const checkChildActive = (childSection: TocSection): boolean => {
        if (activeSection === childSection.item.href.replace("#", "")) return true;
        return childSection.children.some(checkChildActive);
      };
      return checkChildActive(child);
    });
    
    const shouldHighlight = isActive || (hasChildren && isCollapsedSection && isChildActive);
    ...
    return (
      <li key={section.index} className={cn("transition-all duration-200", indentClasses[Math.min(level, 5)])}>
        <div className="flex items-center group">
          ...
          <Link 
            ...
            className={cn(
              ...
              shouldHighlight && "bg-primary/10 border-l-primary text-primary font-medium"
            )}
          >
            <span className={cn(
              "block leading-tight transition-colors",
              textSizes[Math.min(item.depth - 1, 5)],
              shouldHighlight ? "text-primary" : "text-foreground/80 hover:text-foreground"
            )}>
              {item.value}
            </span>
          </Link>
