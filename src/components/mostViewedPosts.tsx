...
export async function MostViewedPosts({ postNum }: { postNum: number }) {
  ...
  if (viewsResult.isErr()) {
    console.error(
      "Error in getting the view count in MostViewedPosts:",
  ...
  if (posts.isErr()) {
    console.error(
      "Error in getting the posts in MostViewedPosts:",
      posts.error.message,
    );
    return (
      <p className="font-normal leading-7 not-first:mt-6 text-destructive">
  ...
  return (
    <ul className="flex flex-col gap-2">
      {posts.value.slice(0, postNum).map((post) => (
        <li
          key={post.slug}
          className="group pl-0 hover:pl-2 transition-all animate-in flex flex-row items-start"
        >
...
export async function MostViewedPostsFallback({
  postNum,
}: {
  postNum: number;
}) {
  ...
  for (let i = 0; i < postNum; i++) {
    posts.push(<Skeleton key={i} className="w-full h-6" />);
  }

  return <ul className="flex flex-col gap-3">{posts}</ul>;
}
